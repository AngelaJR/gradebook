...
var getValueByDataKey = function getValueByDataKey(obj, dataKey, defaultValue) {
  if (_lodash.default.isNil(obj) || _lodash.default.isNil(dataKey)) {
    return defaultValue;
  }

  if ((0, _DataUtils.isNumOrStr)(dataKey)) {
    return _lodash.default.get(obj, dataKey, defaultValue);
  }

  if (_lodash.default.isFunction(dataKey)) {
    return dataKey(obj);
  }
...
var getDomainOfDataByKey = function getDomainOfDataByKey(data, key, type, filterNil) {
  var flattenData = _lodash.default.flatMap(data, function (entry) {
  ...
  var validateData = filterNil ? flattenData.filter(function (entry) {
    return !_lodash.default.isNil(entry);
  }) : flattenData;
    ...
    for (var j = 0, sLen = stackIds.length; j < sLen; j++) {
      ...
      if (barItems && barItems.length) {
        ...
        result[cateId].push({
          ...
          barSize: _lodash.default.isNil(selfSize) ? globalSize : selfSize
        });
      }
    }
  }
...
var getDomainOfErrorBars = function getDomainOfErrorBars(data, item, dataKey, axisType) {
  ...
  var errorBars = (0, _ReactUtils.findAllByType)(children, _ErrorBar.default).filter(function (errorBarChild) {
    ...
    return _lodash.default.isNil(direction) || _lodash.default.isNil(axisType) ? true : axisType.indexOf(direction) >= 0;
  });
  ...
  if (errorBars && errorBars.length) {
    ...
    return data.reduce(function (result, entry) {
      ...
      var mainValue = _lodash.default.isArray(entryValue) ? [_lodash.default.min(entryValue), _lodash.default.max(entryValue)] : [entryValue, entryValue];
      var errorDomain = keys.reduce(function (prevErrorArr, k) {
        var errorValue = getValueByDataKey(entry, k, 0);
        var lowerValue = mainValue[0] - Math.abs(_lodash.default.isArray(errorValue) ? errorValue[0] : errorValue);
        var upperValue = mainValue[1] + Math.abs(_lodash.default.isArray(errorValue) ? errorValue[1] : errorValue);
...
var combineEventHandlers = function combineEventHandlers(defaultHandler, parentHandler, childHandler) {
  ...
  if (_lodash.default.isFunction(childHandler)) {
    customizedHandler = childHandler;
  } else if (_lodash.default.isFunction(parentHandler)) {
    customizedHandler = parentHandler;
  }

  if (_lodash.default.isFunction(defaultHandler) || customizedHandler) {
    return function (arg1, arg2, arg3, arg4) {
      if (_lodash.default.isFunction(defaultHandler)) {
        defaultHandler(arg1, arg2, arg3, arg4);
      }

      if (_lodash.default.isFunction(customizedHandler)) {
...
var parseScale = function parseScale(axis, chartType) {
  ...
  if (_lodash.default.isString(scale)) {
  ...
  return _lodash.default.isFunction(scale) ? {
    scale: scale
  } : {
    scale: d3Scales.scalePoint(),
    realScaleType: 'point'
...
var offsetSign = function offsetSign(series) {
  ...
  for (var j = 0, m = series[0].length; j < m; ++j) {
    ...
    for (var i = 0; i < n; ++i) {
      var value = _lodash.default.isNaN(series[i][j][1]) ? series[i][j][0] : series[i][j][1];
...
var getCateCoordinateOfLine = function getCateCoordinateOfLine(_ref6) {
  ...
  if (axis.type === 'category') {
    ...
    if (!axis.allowDuplicatedCategory && axis.dataKey && !_lodash.default.isNil(entry[axis.dataKey])) {
  ...
  return !_lodash.default.isNil(value) ? axis.scale(value) : null;
};
...
var getCateCoordinateOfBar = function getCateCoordinateOfBar(_ref7) {
  ...
  return !_lodash.default.isNil(value) ? axis.scale(value) - bandSize / 2 + offset : null;
};
...
var parseSpecifiedDomain = function parseSpecifiedDomain(specifiedDomain, dataDomain, allowDataOverflow) {
  if (!_lodash.default.isArray(specifiedDomain)) {
    return dataDomain;
  }
  ...
  } else if (_lodash.default.isFunction(specifiedDomain[0])) {
  ...
  } else if (_lodash.default.isFunction(specifiedDomain[1])) {
...
var getBandSizeOfAxis = function getBandSizeOfAxis(axis, ticks) {
  ...
  if (axis && ticks && ticks.length >= 2) {
    var orderedTicks = _lodash.default.sortBy(ticks, function (o) {
      return o.coordinate;
    });
...
var parseDomainOfCategoryAxis = function parseDomainOfCategoryAxis(specifiedDomain, calculatedDomain, axisChild) {
  ...
  if (_lodash.default.isEqual(specifiedDomain, _lodash.default.get(axisChild, 'type.defaultProps.domain'))) {
    return calculatedDomain;
  }
