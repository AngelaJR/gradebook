...
var getPresentationAttributes = function getPresentationAttributes(el) {
  if (!el || _lodash.default.isFunction(el)) {
    return null;
  }
...
var filterEventsOfChild = function filterEventsOfChild(props, data, index) {
  ...
  for (var i in props) {
    if ({}.hasOwnProperty.call(props, i) && EVENT_ATTRIBUTES[i] && _lodash.default.isFunction(props[i])) {
      if (!out) out = {};
...
var isSvgElement = function isSvgElement(child) {
  return child && child.type && _lodash.default.isString(child.type) && SVG_TAGS.indexOf(child.type) >= 0;
};
/**
...
var filterSvgElements = function filterSvgElements(children) {
  ...
  _react.default.Children.forEach(children, function (entry) {
    if (entry && entry.type && _lodash.default.isString(entry.type) && SVG_TAGS.indexOf(entry.type) >= 0) {
...
var isSingleChildEqual = function isSingleChildEqual(nextChild, prevChild) {
  if (_lodash.default.isNil(nextChild) && _lodash.default.isNil(prevChild)) {
    return true;
  }
...
var isChildrenEqual = function isChildrenEqual(nextChildren, prevChildren) {
  ...
  if (count === 1) {
    return isSingleChildEqual(_lodash.default.isArray(nextChildren) ? nextChildren[0] : nextChildren, _lodash.default.isArray(prevChildren) ? prevChildren[0] : prevChildren);
  }
  ...
  for (var i = 0; i < count; i++) {
    ...
    if (_lodash.default.isArray(nextChild) || _lodash.default.isArray(prevChild)) {
  ...
  _react.Children.forEach(children, function (child, index) {
    if (child && isSvgElement(child)) {
      ...
      if (once && !record[displayName] || !once) {
        ...
        if (_lodash.default.isArray(results)) {
