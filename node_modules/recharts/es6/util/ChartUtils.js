...
export var getDomainOfDataByKey = function getDomainOfDataByKey(data, key, type, filterNil) {
  var flattenData = _.flatMap(data, function (entry) {
...
export var getDomainOfErrorBars = function getDomainOfErrorBars(data, item, dataKey, axisType) {
  ...
  if (errorBars && errorBars.length) {
    ...
    return data.reduce(function (result, entry) {
      ...
      var mainValue = _.isArray(entryValue) ? [_.min(entryValue), _.max(entryValue)] : [entryValue, entryValue];
      var errorDomain = keys.reduce(function (prevErrorArr, k) {
        var errorValue = getValueByDataKey(entry, k, 0);
        var lowerValue = mainValue[0] - Math.abs(_.isArray(errorValue) ? errorValue[0] : errorValue);
        var upperValue = mainValue[1] + Math.abs(_.isArray(errorValue) ? errorValue[1] : errorValue);
...
export var offsetSign = function offsetSign(series) {
  ...
  for (var j = 0, m = series[0].length; j < m; ++j) {
    ...
    for (var i = 0; i < n; ++i) {
      var value = _.isNaN(series[i][j][1]) ? series[i][j][0] : series[i][j][1];
...
export var getBandSizeOfAxis = function getBandSizeOfAxis(axis, ticks) {
  ...
  if (axis && ticks && ticks.length >= 2) {
    var orderedTicks = _.sortBy(ticks, function (o) {
      return o.coordinate;
    });
